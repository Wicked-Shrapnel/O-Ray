...
var VIEW_TYPE_TEST_PLAN = "test-plan-view";
var TEST_PLAN_EXTENSION = "testplan";
var DEFAULT_SETTINGS = { defaultFolder: "", defaultProjectId: "", projects: [] };
var STATUS_ORDER = ["none", "executing", "passing", "failed"];
var STATUS_LABEL = { none: "Not run", executing: "Executing", passing: "Passing", failed: "Failed" };
function isPlanStatus(value) { return typeof value === "string" && STATUS_ORDER.includes(value); }
function makeId() { return Date.now().toString(36) + Math.random().toString(36).slice(2, 8); }
function emptyPlan(title, project) {
  return { title, description: "", status: "none", projectId: project?.id ?? "", projectName: project?.name ?? "", steps: [], deletedSteps: [] };
}
function emptyStep() { return { id: makeId(), action: "", data: "", expectedResult: "" }; }
function normalizeStep(step) {
  return { id: step.id ?? makeId(), action: step.action ?? "", data: step.data ?? "", expectedResult: step.expectedResult ?? "" };
}
function normalizeDeletedStep(record) {
  return {
    id: record.id ?? makeId(),
    originalIndex: typeof record.originalIndex === "number" && Number.isFinite(record.originalIndex) ? Math.max(0, Math.floor(record.originalIndex)) : 0,
  ...
  getItems() {
    ...
    const recurse = (folder) => {
      ...
      for (const child of folder.children) {
        if (child instanceof obsidian.TFolder) recurse(child);
      }
    };
    recurse(this.app.vault.getRoot());
    return folders;
  }
  getItemText(folder) { return folder.path === "/" ? "(Vault root)" : folder.path; }
  onChooseItem(folder) { this.onChoose(folder); }
}
class ProjectChoiceModal extends obsidian.Modal {
  ...
  onOpen() {
    ...
    contentEl.createEl("p", { cls: "tp-project-modal-desc", text: "Choose a project for this plan." });
    ...
    this.newProjectPanel.createEl("div", { cls: "tp-new-project-label", text: "Create a new project here" });
    this.newProjectInput = this.newProjectPanel.createEl("input", { cls: "tp-new-project-input", type: "text", value: "New project" });
...
class ConfirmDeleteModal extends obsidian.Modal {
  constructor(app, stepLabel, onConfirm) {
    super(app);
...
class RestoreStepConflictModal extends obsidian.Modal {
  constructor(app, stepLabel, onReplace, onSideBySide) {
    super(app);
    this.stepLabel = stepLabel;
...
class TestPlanSettingTab extends obsidian.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.plugin = plugin;
  }
  display() {
    ...
    containerEl.createEl("div", { cls: "tp-folder-current", text: "Current folder: " + (this.plugin.settings.defaultFolder || "Vault default location") });
    new obsidian.Setting(containerEl)
      ...
      .addButton((button) => button.setButtonText("Clear").setDisabled(!this.plugin.settings.defaultFolder).onClick(async () => {
        this.plugin.settings.defaultFolder = "";
        await this.plugin.saveSettings();
        this.display();
      }));
    containerEl.createEl("h3", { text: "Projects", cls: "tp-settings-heading" });
    containerEl.createEl("p", { cls: "tp-settings-note", text: "Projects are stored as subfolders inside the default test plan folder." });
    const projectList = containerEl.createDiv({ cls: "tp-project-settings-list" });
    for (const project of this.plugin.settings.projects) this.createProjectSetting(projectList, project);
    new obsidian.Setting(containerEl)
      .setName("Add project")
      .setDesc("Create another project destination for test plans.")
      .addButton((button) => button.setButtonText("Add project").setCta().onClick(async () => {
        this.plugin.settings.projects.push({ id: makeId(), name: "New project", folder: "" });
        await this.plugin.saveSettings();
        this.display();
      }));
  }
  createProjectSetting(parent, project) {
    const row = parent.createDiv({ cls: "tp-project-setting" });
    const details = row.createDiv({ cls: "tp-project-setting-details" });
    const nameInput = details.createEl("input", { cls: "tp-project-name-input", type: "text", value: project.name });
    ...
    details.createEl("div", { cls: "tp-project-folder-label", text: this.plugin.getProjectFolderPath(project) || "Vault default location" });
    ...
    deleteBtn.addEventListener("click", async () => {
      ...
      if (this.plugin.settings.defaultProjectId === project.id) this.plugin.settings.defaultProjectId = "";
      await this.plugin.saveSettings();
      this.display();
    });
  }
}
class TestPlanView extends obsidian.TextFileView {
  ...
  getViewType() { return VIEW_TYPE_TEST_PLAN; }
  getDisplayText() { return this.data_.title.trim() || (this.file ? this.file.basename : "Test plan"); }
  getIcon() { return "list-checks"; }
  getViewData() { return JSON.stringify(this.data_, null, 2); }
  setViewData(data, clear) {
    if (clear || !data.trim()) this.data_ = emptyPlan(this.file ? this.file.basename : "Untitled Test Plan");
    else {
      try {
        ...
        this.data_ = {
          ...
          steps: Array.isArray(parsed.steps) ? parsed.steps.map((s) => normalizeStep(s)) : [],
          deletedSteps: Array.isArray(parsed.deletedSteps) ? parsed.deletedSteps.map((record) => normalizeDeletedStep(record)) : []
        };
      } catch (e) {
  ...
  clear() { this.hasLoadedData = false; }
  async onLoadFile(file) { await super.onLoadFile(file); }
  async onClose() {
  ...
  async syncTitleToFileName() {
    ...
    const newPath = this.plugin.getAvailablePath(obsidian.normalizePath(folderPath ? `${folderPath}/${safeTitle}` : safeTitle), TEST_PLAN_EXTENSION, this.file.path);
  ...
  normalizePlanData(data) {
    return {
      ...
      steps: Array.isArray(data.steps) ? data.steps.map((s) => normalizeStep(s)) : [],
      deletedSteps: Array.isArray(data.deletedSteps) ? data.deletedSteps.map((record) => normalizeDeletedStep(record)) : []
    };
  }
  planDataScore(data) {
    let score = 0;
  ...
  restoreDraftBackup() {
    ...
    if (!key || !this.app.loadLocalStorage) return;
    try {
  ...
  saveDraftBackup() {
    const key = this.getDraftBackupKey();
    if (!key || !this.app.saveLocalStorage) return;
    this.app.saveLocalStorage(key, JSON.stringify({ updatedAt: Date.now(), data: this.data_ }));
  }
  refreshLeafTitle() {
  ...
  createCopyButton(parent, label, getValue) {
    const button = parent.createEl("button", { cls: "tp-section-copy-btn" });
    obsidian.setIcon(button, "clipboard");
    ...
    button.addEventListener("click", async (evt) => {
      ...
      if (!value.trim()) {
        new obsidian.Notice("Nothing to copy for " + label + ".", 2500);
        return;
      }
      await navigator.clipboard.writeText(value);
      new obsidian.Notice("Copied " + label + ".", 2500);
    });
    return button;
  }
  ...
  renderLinkedText(parent, text) {
    ...
    while ((match = linkPattern.exec(source)) !== null) {
      ...
      const link = parent.createEl("a", { text: rawUrl, attr: { href, target: "_blank", rel: "noopener noreferrer" } });
  ...
  render() {
    ...
    container.ondragover = (evt) => this.handleDragAutoScroll(evt);
    const statusBar = container.createDiv({ cls: "tp-status-bar" });
    const statusBadge = statusBar.createDiv({ cls: "tp-status-badge tp-status-" + this.data_.status, text: STATUS_LABEL[this.data_.status] });
    statusBadge.addEventListener("click", (evt) => {
      const menu = new obsidian.Menu();
      for (const s of STATUS_ORDER) {
        menu.addItem((item) => item.setTitle(STATUS_LABEL[s]).setChecked(s === this.data_.status).onClick(() => {
          this.data_.status = s;
          this.markDirty();
          void this.flushFileSave();
          this.render();
    ...
    const projectBtn = statusBar.createEl("button", { cls: "tp-project-badge", text: this.data_.projectName || "No project" });
    projectBtn.addEventListener("click", (evt) => this.showProjectMenu(evt));
    const titleInput = container.createEl("input", { cls: "tp-title-input", type: "text", value: this.data_.title });
    ...
    const descEl = container.createEl("textarea", { cls: "tp-description", attr: { placeholder: "Describe what this test plan covers...", rows: "3" } });
    ...
    if (this.data_.steps.length === 0) {
      ...
      const addFirstBtn = emptyState.createEl("button", { cls: "tp-add-step-btn tp-add-first-btn", text: "+ Add first step" });
  ...
  focusStepAfterMove(stepId) {
    window.requestAnimationFrame(() => {
      window.requestAnimationFrame(() => {
        const stepEl = this.contentEl.querySelector(`[data-step-id="${stepId}"]`);
        if (!stepEl) return;
  ...
  renderStep(parent, step, index) {
    const isEditing = this.editingStepId === step.id;
    const stepEl = parent.createDiv({ cls: "tp-step" + (isEditing ? " tp-step-expanded" : " tp-step-compact") });
  ...
  createEditableField(parent, step, field, label, placeholder) {
    if (field === "id") return;
    ...
    const textarea = wrap.createEl("textarea", { cls: "tp-field-input", attr: { placeholder, rows: "2" } });
  ...
  createPreviewRow(parent, label, value, linkify, onEdit) {
  ...
  archiveDeletedStep(index) {
    const [deletedStep] = this.data_.steps.splice(index, 1);
    if (!deletedStep) return;
    this.data_.deletedSteps.unshift({ id: makeId(), originalIndex: index, deletedAt: Date.now(), step: { ...deletedStep } });
    this.data_.deletedSteps = this.data_.deletedSteps.slice(0, 25);
    this.markDirty();
  }
  restoreDeletedStep(index, mode = "insert") {
  ...
  requestRestoreDeletedStep(index) {
    ...
    new RestoreStepConflictModal(this.app, "Step " + (record.originalIndex + 1), () => {
  ...
  renderDeletedSteps(parent) {
    ...
    this.data_.deletedSteps.forEach((record, index) => {
      ...
      itemHeader.addEventListener("click", () => {
        if (isExpanded) this.expandedDeletedStepIds.delete(record.id);
        else this.expandedDeletedStepIds.add(record.id);
        this.renderPreservingScroll();
      });
      const itemText = itemHeader.createDiv({ cls: "tp-deleted-step-text" });
      itemText.createEl("div", { cls: "tp-deleted-step-title", text: "Step " + (record.originalIndex + 1) });
      itemText.createEl("div", { cls: "tp-deleted-step-summary", text: previewText(record.step.action, "No action") });
  ...
  renderDeletedStepDetail(parent, label, value, linkify = false) {
  ...
  addStepAt(index) {
    if (this.editingStepId) {
      new obsidian.Notice("Confirm or cancel the current step before adding another.", 3500);
      return;
    }
  ...
  moveStep(fromIndex, toIndex, insertAfter = false) {
  ...
  showProjectMenu(evt) {
    const menu = new obsidian.Menu();
    for (const project of this.plugin.settings.projects) {
      menu.addItem((item) => item.setTitle(project.name || "Untitled project").setChecked(project.id === this.data_.projectId).onClick(() => void this.changeProject(project)));
    }
    menu.addSeparator();
    menu.addItem((item) => item.setTitle("No project").setChecked(!this.data_.projectId).onClick(() => void this.changeProject()));
    menu.showAtMouseEvent(evt);
  }
...
class TestPlanPlugin extends obsidian.Plugin {
  async onload() {
    ...
    this.addCommand({ id: "create-new-test-plan", name: "Create new test plan", callback: () => this.openNewPlanPicker() });
  }
  async loadSettings() {
    this.settings = Object.assign({}, DEFAULT_SETTINGS, await this.loadData());
    if (!Array.isArray(this.settings.projects)) this.settings.projects = [];
  }
  ...
  sanitizeFolderSegment(name) {
    return (name.trim() || "Untitled project").replace(/[\\/:*?"<>|#^[\]]+/g, "-").replace(/\s+/g, " ").trim();
  }
  getProjectFolderPath(project) {
    ...
    return obsidian.normalizePath(mainFolder ? `${mainFolder}/${projectFolder}` : projectFolder);
  }
  ...
  async movePlanToProjectFolder(file, project) {
    ...
    const newPath = this.getAvailablePath(obsidian.normalizePath(`${targetFolder}/${file.basename}`), TEST_PLAN_EXTENSION, file.path);
    if (newPath === file.path) return;
    await this.app.fileManager.renameFile(file, newPath);
  }
  async createNewTestPlan(project) {
  ...
  getAvailablePath(basePath, extension, allowedPath = "") {
...
module.exports = { default: TestPlanPlugin, VIEW_TYPE_TEST_PLAN, TEST_PLAN_EXTENSION };
